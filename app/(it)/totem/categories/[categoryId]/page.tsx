...
export async function generateMetadata({
  ...
  const category = resolveCategoryPageData(categoryId, "it");

  if (!category) {
    return {};
  }

  return createCategoryPageMetadata(category, "totem", "it");
}

export default async function TotemCategoryPage({
  params
}: {
  params: Promise<{ categoryId: string }>;
}) {
  ...
  return <CategoryRouteView category={category} locale="it" />;
}
