...
export async function generateMetadata({
  ...
  const pageData = resolveBusinessPageData(categoryId, businessId, "en");

  if (!pageData) {
    return {};
  }

  return createBusinessPageMetadata(categoryId, pageData.business, "totem", "en");
}

export default async function TotemEnglishBusinessPage({
  params
}: {
  params: Promise<{ categoryId: string; businessId: string }>;
}) {
  ...
  return <BusinessRouteView business={pageData.business} category={pageData.category} includeStructuredData={false} locale="en" />;
}
