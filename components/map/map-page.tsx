import { getLocalizedPointOfInterestLegend } from "@/features/map/map.i18n";
...
export function MapPage({ locale }: { locale: Locale }) {
  ...
  const localizedLegend = getLocalizedPointOfInterestLegend(locale);
  const isDevelopment = process.env.NODE_ENV === "development";

  return (
    <section className="flex flex-1 flex-col gap-8">
      <BackLink href="/" label={messages.map.backHome} />

      <section className="glass-panel soft-outline rounded-[2rem] border p-7">
        <div className="flex flex-wrap items-center gap-4">
          <h1 className="text-4xl font-semibold text-navy-950">{messages.map.title}</h1>
        </div>
      </section>

      <section className="glass-panel soft-outline rounded-[1.7rem] border px-5 py-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-navy-900/45">{messages.map.legendTitle}</p>
          <ul className="flex flex-wrap items-center gap-3" aria-label={messages.map.legendAriaLabel}>
            {localizedLegend.map(({ key, label, color }) => (
              <li
                key={key}
